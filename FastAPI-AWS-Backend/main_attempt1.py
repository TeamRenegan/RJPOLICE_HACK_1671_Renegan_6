...
# MongoDB connection
# client = MongoClient("mongodb+srv://ankit:doraemon123@metadata.tjnldk9.mongodb.net/?retryWrites=true&w=majority")
# db = client["renegan"]
# collection = db["videos"]

# Dictionary to store the first frame of each hour
first_frames = {
...
def trim_video(frames, start_time, end_time):
    ...
    frames_to_include = [frame for frame, timestamp in frames if start_time - 5 <= timestamp <= end_time + 5]

    # Define the codec and create a VideoWriter object
    fourcc = cv2.VideoWriter_fourcc(*'mp4v') 
    out = cv2.VideoWriter('trimmed.mp4', fourcc, 20.0, (640, 480))
...
def save_frames_as_video(frames):
    # Define the codec and create a VideoWriter object
    fourcc = cv2.VideoWriter_fourcc(*'mp4v') 
    out = cv2.VideoWriter('output_att1.mp4', fourcc, cap.get(cv2.CAP_PROP_FPS), (640, 480))
...
async def process_stream():
    ...
    while True:
        ...
        # The frames differ
            cam_disp_time = min(cam_disp_time, current_time) if cam_disp_time is not None else current_time
            current_state = 1
            cam_disp_count += 1
            print("CAM displaced")
    ...
    # If the state has changed
        if last_state_of_displacement is not None and current_state != last_state_of_displacement:
        ...
        if current_time >= last_upload_time + 60 * 60:
            ...
            date_str = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            s3_client.upload_file(video_file, "ankit-s3-1", f"complete_footage/{date_str}.mp4")
...
# else:
    ...
    if cam_disp_count > cam_not_disp_count:
        ...
        # db["displacement_notification"].insert_one({"timestamp": cam_disp_time, "cameraId": "123456",})
